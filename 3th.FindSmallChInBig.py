...
'''写的很烂 仅作参考
另外还好给的文本里没有那种重复叠加的像 'XXXxXXXxXXX' 这样的 不然用re.findall是不成的
'''
...
strPattern = '[a-z][A-Z]{3}[a-z][A-Z]{3}[a-z]' # xXXXxXXXx
strPattResult = re.findall(strPattern, text)
lstTemp = []

for strTemp1 in strPattResult:
	strTemp2 = ""
	for chrTemp in strTemp1:
		if(ord(chrTemp) > 91):
			strTemp2 = strTemp2 + chrTemp
	lstTemp.append(strTemp2[1])
